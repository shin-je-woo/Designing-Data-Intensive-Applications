...
- 데이터를 표현하기 위해 노력 → 관계형 모델 고안
- 관계형 모델이 적합하지 않은 문제 해결 → NoSQL
- NoSQL → 문서 데이터베이스와 그래프 데이터베이스
    - 문서 → 데이터가 문서에 포함되어 있고 문서 간의 관계가 거의 없는 사례
    - 그래프 → 모든 것이 잠재적으로 관계가 있는 사례
- 서로 다른 모델끼리 흉내를 낼 수는 있지만 이는 효율적이지 않음
    - 각자 목적에 맞는 솔루션을 사용 해야함
- NoSQL 의 강점 → 스키마가 강제 되지 않아 변화하는 요구사항에 대응하기 쉽다
